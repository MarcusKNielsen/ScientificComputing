...
def Solve(Model, Jacobian, tspan, x0, h0, abstol, reltol, args):
    ...
    while t < tf:
        ...
        while not AcceptStep:
            ...
            xnxt, nfev, njac, nlu = NewtonsMethod(Model, Jacobian, tnxt, dt, x, x, TOL, MaxIter, *args)
            Nfev += nfev
            Njac += njac
            Nlu += nlu
            
            # Calculate the next x using two half steps.
            x1nxt, nfev, njac, nlu = NewtonsMethod(Model, Jacobian, th, dth, x, x, TOL, MaxIter, *args)
            Nfev += nfev
            Njac += njac
            Nlu += nlu
                               # t plus a half step
            x2nxt, nfev, njac, nlu = NewtonsMethod(Model, Jacobian, tnxt, dth, x1nxt, x1nxt, TOL, MaxIter, *args)
            Nfev += nfev
            Njac += njac
            Nlu += nlu
